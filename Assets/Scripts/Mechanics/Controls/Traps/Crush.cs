...
public class Crush : Trap {
    ...
    public override void IdleAction() {
        for (int i = 0; i < vision.container.Count; i++) {
            if (vision.container[i].tag == playerTag) {
                Vector2 targetDirection = Compass.SnapVector(transform.position - vision.container[i].transform.position);
                if (Compass.DirectionVectors.ContainsKey(targetDirection)) {
                    state.direction = Compass.DirectionVectors[targetDirection];
    ...
    public override void ActiveAction() {
        //
        movementVector = Compass.VectorDirections[state.direction];
